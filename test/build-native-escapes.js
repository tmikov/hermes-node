...
//
// The closed world holds in a natively compiled executable exactly as it
// does in a container or a --build-exe executable. Same cases as
// test/build-exe-escapes.js -- the `require`s that are not the loader's
// own, which a module's wrapper parameter shadows and which therefore
// nothing ordinary meets -- ported case for case, so a divergence between
// the two artifact kinds shows up as a difference between two files that
// should read alike. The one structural change from the source file:
// build-native compiles and links directly from the entry, so the
// --build-bundle-then---build-exe pair collapses into one build-native
// invocation.
//
...
// RUN: %hermes-node build-native %t.src/cli.js -o %t.out/app.exe --include=./local --include=dep --kit=%kit_dir > /dev/null
