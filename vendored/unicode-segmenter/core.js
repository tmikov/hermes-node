...
exports.decodeUnicodeFlatData = decodeUnicodeFlatData;
exports.findUnicodeRangeCategory = findUnicodeRangeCategory;
// @ts-check
 ...
 * @return {void}
 */
function decodeUnicodeData(data, cats, emit) {
  let len = data.length,
    i = 0,
    j = 0,
    p = -1;
 ...
 */
function findUnicodeRangeCategory(cp, starts, ends) {
  let lo = 0,
    hi = starts.length - 1;
  while (lo <= hi) {
    let mid = lo + hi >>> 1,
      end = ends[mid];
    if (cp < starts[mid]) hi = mid - 1;else if (cp > end >>> 5) lo = mid + 1;else return end & 31;
  }
  return 0;
}
 ...
 */
function decodeUnicodeFlatData(data) {
