...
Tracks progress on `docs/superpowers/plans/2026-02-24-debugger-plan.md`.
...
- **Files**: modified `include/hermes/node-compat/inspector/inspector_bridge.h`, modified `lib/inspector/inspector_bridge.cpp`, modified `lib/runtime/hermes_node_runtime.cpp`, modified `libjs/shims/inspector-server.js`, modified `hermes/API/napi/hermes_napi.cpp`, modified `lib/embedded-modules/CMakeLists.txt`.
- **What was done**: Implemented `--inspect-brk` which pauses the runtime before user code executes. Five key changes: (1) Added `triggerInboundDrain` callback to `InspectorBridgeContext` that uses `AsyncDebuggerAPI::triggerInterrupt_TS()` to deliver inbound CDP commands even when the runtime is paused (event loop not running during pause). Called from `sendToMain()` in inspector_bridge.cpp. (2) In `hermes_node_runtime.cpp`, pre-enable the debugger domain for `--inspect-brk` (`cdpAgent->enableDebuggerDomain()`), flush the pending runtime task synchronously, then execute `debugger;` via `napi_run_script` to trigger the pause. (3) Added outbound CDP message buffering in `inspector-server.js` for `--inspect-brk`: messages are queued in `pendingMessages[]` when no WebSocket client is connected, and replayed when a client connects. This captures `Debugger.paused` notifications sent before DevTools attaches. (4) Added `compileFlags.debug = true` in `hermes_run_script` (NAPI layer) under `HERMES_ENABLE_DEBUGGER` so eval'd code includes debug info for breakpoints/stepping. (5) Added `-g` flag to embedded module compilation in CMakeLists.txt so all bootstrap modules have debug info.
- **Decisions**:
  -- Used `debugger;` statement (via `napi_run_script`) instead of `coordinator().pause()` for the pause mechanism. `coordinator().pause()` sets an async break flag that is only checked at `AsyncBreakCheck` instructions and `doCall` points in the interpreter. With `pauseOnScriptLoad` enabled, each module load enters the debugger via a `ScriptLoaded` breakpoint before any `doCall` occurs, so the `ExplicitPause` flag was never consumed. The `debugger;` statement goes through the old Debugger's `Debugger` opcode -> `debuggerLoop` -> `didPause` path which reliably pauses.
  -- Added `triggerInboundDrain` as a separate mechanism from the RuntimeTaskRunner's built-in dual delivery. During a pause, the event loop is blocked in `processInterruptWhilePaused`, so `uv_async_send` alone cannot deliver CDP commands. The interrupt-based drain ensures commands like `Debugger.resume` reach the paused runtime.
