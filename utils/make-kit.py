...
def main():
    ...
    ap.add_argument("--ndebug", required=True, choices=("0", "1"),
                    help="whether hermesvm_a was compiled with NDEBUG "
                         "defined -- generated C must match it or "
                         "_SH_MODEL() fails to link; pass "
                         "$<CONFIG:Release,RelWithDebInfo,MinSizeRel>, not a "
                         "hardcoded guess")
    ap.add_argument("argv", nargs=argparse.REMAINDER)
    args = ap.parse_args()
    args.ndebug = args.ndebug == "1"
    ...
    with open(os.path.join(args.kit_dir, KIT_MANIFEST_NAME), "w") as f:
        ...
        #
        # -DNDEBUG is conditional on args.ndebug, NOT hardcoded: the
        # generated C calls _SH_MODEL(), whose symbol name
        # (_sh_model_<HERMESVM_MODEL>) is suffixed _dbg or _rel depending on
        # whether NDEBUG was defined for the TU that computed it (see
        # libhermesvm-config.h.in). hermesvm_a, merged into this same kit,
        # got NDEBUG from CMake's own per-config flags -- defined for
        # Release/RelWithDebInfo/MinSizeRel, not for Debug. Hardcoding
        # -DNDEBUG here asserted a fact about the build that this script does
        # not know, so a kit cut in a Debug configuration (the ASAN
        # configuration this project develops in) compiled every generated
        # unit against the wrong suffix and failed at link time with an
        # undefined _sh_model_..._rel -- exactly the "link error instead of
        # silent corruption" outcome _SH_MODEL() exists to produce, firing
        # for the first time only once something actually linked a native
        # unit. args.ndebug carries the true answer in from CMake, which
        # knows it per the same $<CONFIG:...> logic that decides hermesvm_a's
        # own flags.
        ccflags = ["-fno-strict-aliasing", "-fno-strict-overflow",
                   "-I{kit}/include"]
        if args.ndebug:
    ...
    if stamp:
        ...
        open(stamp, "w").close()
    print("make-kit: %d archives merged into %s, %d SH headers copied"
          % (len(archives), kit_archive, n_headers), file=sys.stderr)
