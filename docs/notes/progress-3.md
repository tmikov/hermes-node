...
Tracks progress on `docs/superpowers/plans/2026-02-16-phase5-networking-plan.md`.
...
### Embedded Bytecode: Pre-compile JS modules and embed in binary
- **Files**: created `embedded-modules.txt`, `tools/wrap-cjs.py`, `tools/gen-embedded-registry.py`, `include/hermes/node-compat/embedded-modules/embedded_modules.h`, `lib/embedded-modules/embedded_modules.cpp`, `lib/embedded-modules/CMakeLists.txt`. Modified `CMakeLists.txt`, `tools/hermes-node/CMakeLists.txt`, `tools/hermes-node/hermes-node.cpp`, `lib/module-loader/module_loader.cpp`, `include/hermes/node-compat/module-loader/module_loader.h`, `libjs/loader.js`, `test/lit.cfg`, `unittests/ModuleLoaderTest.cpp`, `unittests/CMakeLists.txt`.
- **Decisions**:
-- Module manifest (`embedded-modules.txt`) generated by tracing loader.js during full test suite run. ~90 modules total.
-- Build pipeline: `.js` → `wrap-cjs.py` (CJS only) → `hermesc -emit-binary` → `xxd.py` → `.inc` → `#include` in generated registry `.cpp`.
-- Shim resolution at build time: `libjs/shims/<id>.js` takes priority over `libjs-node/<id>.js`.
-- `@bootstrap` prefix in manifest marks non-CJS modules (primordials, loader) that are compiled without CJS wrapper.
-- `alignas(4)` on C arrays matches `hbc::BYTECODE_ALIGNMENT`.
-- No disk fallback for internal modules. `--node-lib-path` CLI argument removed entirely.
-- `loader.js` now receives `loadBytecodeModule(id)` as first arg; tries embedded bytecode first, falls back to `readFileSync` for user scripts only.
-- Unit tests rewritten to test embedded module API directly (findEmbeddedModule, runEmbeddedModule, loadBytecodeModuleCallback) rather than full module chain loading.
- **What was done**: Complete build-time bytecode compilation pipeline. All ~90 internal JS modules pre-compiled to Hermes bytecode and embedded as C arrays in the binary. Runtime loads modules from bytecode via `hermes_run_bytecode()` with zero disk I/O for internal modules. User scripts still loaded from disk. All 49 JS lit tests + 55 unit tests pass. No `--node-lib-path` needed.
