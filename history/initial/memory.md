...
- `InspectorState` struct (in `hermes_node_runtime.cpp`): mutex-protected queues + `uv_async_t` + `std::atomic<bool> asyncActive`. Drains inbound CDP commands (`agent->handleCommand`) and RuntimeTasks (`task(*hermesRT)`) on main thread.
...
- `InspectorBridgeContext` struct (`inspector/inspector_bridge.h`): cross-thread CDP messaging context. Holds outbound queue+mutex+`inspectorAsync`+`inspectorAsyncActive` (main->inspector), pointers to inbound queue+mutex+async (inspector->main), config (host/port/scriptName/sessionId), startup sync, JS callback ref. Null for user runtime, set for inspector runtime via `config.inspectorBridgeContext`.
- **CDP message flow (end-to-end)**: DevTools -> WS -> inspector JS `ws.on('message')` -> `bridge.sendToMain(json)` -> main `inboundQueue` -> `uv_async_send(mainAsync)` -> `onInspectorAsync` -> `cdpAgent->handleCommand(json)` -> CDPAgent outbound callback -> `bridgeCtx->outboundQueue` -> `uv_async_send(inspectorAsync)` -> `onOutboundAsync` -> JS `messageCallback(msg)` -> `ws.send(msg)` -> DevTools.
- **bridgeCtx must be allocated before CDPAgent**: The CDPAgent messageCallback lambda captures `bridgeCtx` by pointer. Move bridgeCtx allocation before `CDPAgent::create()` in `runHermesNode()`.
- **Two async guards in InspectorBridgeContext**: `inspectorAsyncActive` (guards `uv_async_send` to inspector loop) and `canSendShutdown` (guards shutdown async). Both set false before `eventLoop.close()` on the inspector runtime.
- `inspector_bridge` binding: `sendToMain`, `setMessageCallback`, `getConfig`, `notifyReady`, `setShutdownCallback`. Empty object when no bridge context. Library: `hermesNodeInspector` in `lib/inspector/`.
...
- **Inspector thread lifecycle**: `std::thread` in `runHermesNode()`. `InspectorBridgeContext` heap-allocated, populated from `InspectorState`. Wait on `readyCv` for server ready. Shutdown: `uv_async_send(&shutdownAsync)` -> JS callback closes servers -> loop exits -> thread joined. Mutex-protected `canSendShutdown` flag prevents use-after-close races. `canSendShutdown = false` set in inspector runtime's cleanup BEFORE `eventLoop.close()`.
- **Inspector shutdown gotcha**: `UvEventLoop::close()` uses `uv_run(UV_RUN_DEFAULT)` which blocks on active handles. Must NOT use `uv_stop()` for inspector shutdown -- it gets cleared before `eventLoop.close()` re-enters `uv_run`. Use JS callback to close servers instead, letting the loop exit naturally.
...
- **--inspect-brk pause mechanism**: Uses `napi_run_script(env, "debugger;")` before user code. Cannot use `coordinator().pause()` because its async break flag is never consumed: with `pauseOnScriptLoad` enabled, each module load enters the debugger via a ScriptLoaded breakpoint before any `doCall` occurs, so the ExplicitPause flag is skipped. The `debugger;` statement goes through the old Debugger's opcode -> debuggerLoop -> didPause -> coordinator -> setPaused path.
- **--inspect-brk task flush**: `enableDebuggerDomain()` enqueues a RuntimeTask to `inspectorState.runtimeTasks`. Must flush synchronously before `debugger;` so the coordinator's event callback is registered. Without this, the DebuggerStatement event has no handler.
- **triggerInboundDrain**: `InspectorBridgeContext::triggerInboundDrain` uses `AsyncDebuggerAPI::triggerInterrupt_TS()` to deliver CDP commands during pause (event loop not running). Called from `sendToMain()` in inspector_bridge.cpp.
- **Inspector JS message buffering**: `inspector-server.js` buffers outbound CDP messages in `pendingMessages[]` when no WS client is connected. Replayed on client connect. Needed for `--inspect-brk`: `Debugger.paused` is sent before DevTools attaches.
- **NAPI debug info**: `hermes_run_script` must set `compileFlags.debug = true` under `HERMES_ENABLE_DEBUGGER` (in `hermes/API/napi/hermes_napi.cpp`). Without this, eval'd code has no debug info and the debugger can't pause/step. The JSI API (hermes.cpp) and `eval()` (eval.cpp) both set this flag but NAPI was missing it.
- **Embedded module debug info**: `-g` flag added to `JS_COMPILER_FLAGS` in `lib/embedded-modules/CMakeLists.txt` so all bootstrap modules include debug info. Required for the debugger to set breakpoints in bootstrap code.
