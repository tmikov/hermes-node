...
static napi_value pushAsyncContext(napi_env env, napi_callback_info /*info*/) {
  return nullptr;
}
...
static napi_value clearAsyncIdStack(napi_env env, napi_callback_info /*info*/) {
  return nullptr;
}
...
/// hooks. No-op.
static napi_value setPromiseHooks(napi_env env, napi_callback_info /*info*/) {
  return nullptr;
}

/// getPromiseHooks() — returns current promise hooks. Returns undefined.
static napi_value getPromiseHooks(napi_env env, napi_callback_info /*info*/) {
  napi_value undef;
  napi_get_undefined(env, &undef);
  return undef;
}
...
napi_value initAsyncWrapBinding(napi_env env, napi_value exports) {
  ...
  // --- async_hook_fields: Uint32Array(kFieldsCount), all zeros ---
  {
    ...
    NAPI_CALL(napi_create_typedarray(
        env, napi_uint32_array, kFieldsCount, ab, 0, &arr));
  ...
  // --- async_ids_stack: Float64Array(0), empty ---
  {
    ...
    napi_value arr;
    NAPI_CALL(napi_create_typedarray(env, napi_float64_array, 0, ab, 0, &arr));
