...
function initHermesParserWASM() {
  ...
  const factoryResult = (0, _HermesParserWASM.default)(wasmModule);

  if (typeof (factoryResult == null ? void 0 : factoryResult.then) === 'function') {
    // Nothing awaits this promise, so report a rejected initialization rather
    // than leaving it unhandled.
    factoryResult.then(undefined, error => {
  ...
  if (typeof wasmModule.cwrap !== 'function') {
    throw new Error('HermesParserWASM did not initialize synchronously. It must be built ' + 'with `-sSINGLE_FILE=1 -sWASM_ASYNC_COMPILATION=0`.');
  }
